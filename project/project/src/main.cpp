...
int main()
{
	...
	VBO vbo(sizeof(cube), cube);

	VAO vao;
	vao.bind();
	vao.link_attrib(vbo, 0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);
	vao.unbind();

	VAO light_vao;
	light_vao.bind();
	light_vao.link_attrib(vbo, 0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);
	light_vao.unbind();
